...
class ControlCampaignApiTest extends TestCase
{
    ...
    public function test_presentation_state_is_revisioned_authorized_and_preserved_by_adoption_preflight(): void
    {
        ...
        $state = ['scene_id' => $ids['scene'], 'backdrop_asset_id' => $ids['asset'], 'music_cue_id' => $ids['music'], 'music_playback' => ['status' => 'paused', 'position_seconds' => 14.5, 'position_command_id' => (string) Str::uuid7(), 'loop' => false, 'volume' => .35, 'fade_duration_ms' => 500], 'video_cue_id' => $ids['video'], 'stage_entries' => [['npc_id' => $ids['npc'], 'npc_state_id' => $ids['state'], 'position_x' => -0.5, 'position_y' => 1.5, 'scale' => 1, 'layer_order' => 2, 'facing' => 'left']]];
        $payload = ['command_id' => (string) Str::uuid7(), 'expected_revision' => 1, 'state' => $state];
        $this->putJson($base, $payload)->assertOk()->assertJsonPath('data.revision', 2)->assertJsonPath('data.state.scene_id', $ids['scene'])->assertJsonPath('data.state.stage_entries.0.position_x', -0.5)->assertJsonPath('data.state.stage_entries.0.position_y', 1.5)->assertJsonPath('data.state.music_playback.status', 'paused')->assertJsonPath('data.state.music_playback.position_seconds', 14.5)->assertJsonPath('data.state.music_playback.loop', false)->assertJsonPath('data.state.music_playback.volume', .35)->assertJsonPath('data.state.music_playback.fade_duration_ms', 500)->assertJsonPath('data.state.standby', null)->assertJsonPath('data.state.standby_status', 'idle')->assertJsonPath('data.state.standby_error', null);
