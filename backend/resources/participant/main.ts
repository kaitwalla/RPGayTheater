...
const ParticipantApp = defineComponent({
    ...
    setup() {
        ...
        const connect = async (): Promise<void> => {
            error.value = '';
            try { await Promise.all([currentMap.start(), loadRoster(), loadPlayerGroups(), loadMessages(), loadPolls(), loadNpcs()]); await loadImage(); }
        ...
        return { playerCode, displayName, role, resumeToken, identity, roster, playerGroups, messages, polls, messageTarget, messageGroupId, messageBody, replyToMessageId, npcs, noteNpcId, noteBody, error, busy, join, resume, claim, sendMessage, replyTo, vote, addNpcNote, editNpcNote, deleteNpcNote, currentMap, imageUrl };
    },
    template: `
