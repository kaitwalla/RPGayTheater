...
const SessionsView = defineComponent({
    ...
    setup() {
        ...
        const savePresentationEntries = async (entries: PresentationStateEntry[], presetId = presentation.value?.state.stage_preset_id ?? null, backdropId = presentation.value?.state.backdrop_asset_id ?? null): Promise<void> => { const session = selectedSession(); if (!session || !presentation.value) return; const state = presentation.value.state; busy.value = true; try { presentation.value = (await api<ApiResponse<PresentationSnapshot>>(`/api/control/v1/campaigns/${campaignId}/sessions/${session.id}/presentation-state`, { method: 'PUT', body: JSON.stringify({ command_id: commandId(), expected_revision: presentation.value.revision, state: { scene_id: state.scene_id, backdrop_asset_id: backdropId, music_cue_id: state.music_cue_id, video_cue_id: state.video_cue_id, stage_preset_id: presetId, stage_entries: entries } }) })).data; stagePresetId.value = presetId ?? ''; await loadPresentationAssets(); } catch (reason) { error.value = reason instanceof Error ? reason.message : 'Unable to update staged NPCs.'; await loadWorkspace(); } finally { busy.value = false; } };
