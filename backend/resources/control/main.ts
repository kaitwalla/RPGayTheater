...
type PinnedNpcState = { id: string; npc_id: string; asset_id: string };
...
const SessionsView = defineComponent({
    ...
    setup() {
        ...
        const sessions = ref<LiveSessionRecord[]>([]); const revisions = ref<CampaignRevision[]>([]); const maps = ref<PinnedMap[]>([]); const scenes = ref<PinnedScene[]>([]); const npcs = ref<PinnedNpc[]>([]); const npcStates = ref<PinnedNpcState[]>([]); const presetEntries = ref<PinnedStagePresetEntry[]>([]); const selectedSessionId = ref(''); const createRevisionId = ref(''); const progressMode = ref<'fresh' | 'resume'>('fresh'); const playerMap = ref<PlayerMapState | null>(null); const progress = ref<MapProgress | null>(null); const presentation = ref<PresentationSnapshot | null>(null); const presentationAssetUrls = ref<Record<string, string>>({}); const standbySceneId = ref(''); const brushMode = ref<'reveal' | 'hide'>('reveal'); const brushX = ref(.5); const brushY = ref(.5); const brushRadius = ref(.1); const imageUrl = ref(''); const error = ref(''); const busy = ref(false);
        ...
        const movePresentationEntry = async (moved: PresentationStageEntry): Promise<void> => { const session = selectedSession(); if (!session || !presentation.value) return; const state = presentation.value.state; const entries = state.stage_entries.map((entry) => entry.npc_id === moved.npc_id && entry.npc_state_id === moved.npc_state_id && entry.layer_order === moved.layer_order ? { ...entry, position_x: moved.position_x, position_y: moved.position_y } : entry); busy.value = true; try { presentation.value = (await api<ApiResponse<PresentationSnapshot>>(`/api/control/v1/campaigns/${campaignId}/sessions/${session.id}/presentation-state`, { method: 'PUT', body: JSON.stringify({ command_id: commandId(), expected_revision: presentation.value.revision, state: { scene_id: state.scene_id, backdrop_asset_id: state.backdrop_asset_id, music_cue_id: state.music_cue_id, video_cue_id: state.video_cue_id, stage_entries: entries } }) })).data; } catch (reason) { error.value = reason instanceof Error ? reason.message : 'Unable to move this staged NPC.'; await loadWorkspace(); } finally { busy.value = false; } };
        onMounted(load); return { sessions, revisions, maps, scenes, selectedSessionId, createRevisionId, progressMode, playerMap, progress, presentation, presentationAssetUrls, activeEntries, activeScene, standbySceneId, brushMode, brushX, brushY, brushRadius, imageUrl, error, busy, selectedMap, loadWorkspace, create, setMap, selectMap, brush, brushAt, reset, saveTokens, moveTokens, standby, go, movePresentationEntry, back: () => router.push('/') };
    },
    template: `<main class="shell stack">
        ...
        <section v-if="presentation" class="panel stack"><h2>Presentation preview</h2><p class="muted">Drag a staged NPC to reposition it. The move saves as a revisioned active-stage update.</p><PresentationStage :backdrop-asset-id="presentation.state.backdrop_asset_id" :transition="activeScene?.transition || 'cut'" :transition-duration-ms="activeScene?.transition_duration_ms || 0" :entries="activeEntries" :asset-urls="presentationAssetUrls" editable @move-entry="movePresentationEntry" /></section>
