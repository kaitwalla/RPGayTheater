...
type PresentationSnapshot = { revision: number; state: { scene_id: string | null; backdrop_asset_id: string | null; music_cue_id: string | null; video_cue_id: string | null; stage_entries: unknown[]; standby: unknown | null; standby_status: 'idle' | 'preparing' | 'ready' | 'error'; standby_error: string | null } };
type PinnedScene = { id: string; name: string; primary_backdrop_asset_id: string | null; default_music_cue_id: string | null };
...
const SessionsView = defineComponent({
    components: { ControlMapStage },
    setup() {
        const route = useRoute(); const router = useRouter(); const campaignId = String(route.params.campaign);
        const sessions = ref<LiveSessionRecord[]>([]); const revisions = ref<CampaignRevision[]>([]); const maps = ref<PinnedMap[]>([]); const scenes = ref<PinnedScene[]>([]); const selectedSessionId = ref(''); const createRevisionId = ref(''); const progressMode = ref<'fresh' | 'resume'>('fresh'); const playerMap = ref<PlayerMapState | null>(null); const progress = ref<MapProgress | null>(null); const presentation = ref<PresentationSnapshot | null>(null); const standbySceneId = ref(''); const brushMode = ref<'reveal' | 'hide'>('reveal'); const brushX = ref(.5); const brushY = ref(.5); const brushRadius = ref(.1); const imageUrl = ref(''); const error = ref(''); const busy = ref(false);
        ...
        const loadWorkspace = async (): Promise<void> => { const session = selectedSession(); if (!session) { maps.value = []; scenes.value = []; playerMap.value = null; progress.value = null; presentation.value = null; imageUrl.value = ''; return; } const [revision, state, presentationState] = await Promise.all([api<ApiResponse<{ manifest: { maps?: PinnedMap[]; scenes?: PinnedScene[] } }>>(`/api/control/v1/campaigns/${campaignId}/revisions/${session.campaign_revision_id}`), api<ApiResponse<PlayerMapState>>(`/api/control/v1/campaigns/${campaignId}/sessions/${session.id}/player-map`), api<ApiResponse<PresentationSnapshot>>(`/api/control/v1/campaigns/${campaignId}/sessions/${session.id}/presentation-state`)]); maps.value = revision.data.manifest.maps ?? []; scenes.value = revision.data.manifest.scenes ?? []; playerMap.value = state.data; presentation.value = presentationState.data; standbySceneId.value ||= scenes.value[0]?.id ?? ''; const map = selectedMap(); progress.value = state.data.map_id ? (await api<ApiResponse<MapProgress>>(`/api/control/v1/campaigns/${campaignId}/sessions/${session.id}/maps/${state.data.map_id}/progress`)).data : null; imageUrl.value = map ? (await api<ApiResponse<{ url: string }>>(`/api/control/v1/campaigns/${campaignId}/assets/${map.image_asset_id}/read`)).data.url : ''; };
