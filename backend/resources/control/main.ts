...
type MapProgress = { revision: number; tokens: MapToken[]; fog: { default_visibility: string; brushes: unknown[] } };
...
type PresentationCue = { scene_id: string | null; backdrop_asset_id: string | null; music_cue_id: string | null; video_cue_id: string | null; stage_preset_id: string | null; stage_entries: PresentationStateEntry[] };
...
type PinnedNpc = { id: string; name: string; normal_asset_id: string; native_facing: 'left' | 'right' };
...
type PinnedAudioCue = { id: string; name: string; kind: 'music' | 'sfx' };
...
const SessionsView = defineComponent({
    ...
    setup() {
        ...
        const sessions = ref<LiveSessionRecord[]>([]); const revisions = ref<CampaignRevision[]>([]); const maps = ref<PinnedMap[]>([]); const scenes = ref<PinnedScene[]>([]); const sceneBackdrops = ref<PinnedSceneBackdrop[]>([]); const audioCues = ref<PinnedAudioCue[]>([]); const npcs = ref<PinnedNpc[]>([]); const npcStates = ref<PinnedNpcState[]>([]); const presets = ref<PinnedStagePreset[]>([]); const presetEntries = ref<PinnedStagePresetEntry[]>([]); const selectedSessionId = ref(''); const createRevisionId = ref(''); const progressMode = ref<'fresh' | 'resume'>('fresh'); const playerMap = ref<PlayerMapState | null>(null); const progress = ref<MapProgress | null>(null); const presentation = ref<PresentationSnapshot | null>(null); const presentationAssetUrls = ref<Record<string, string>>({}); const standbySceneId = ref(''); const stagePresetId = ref(''); const stageNpcId = ref(''); const stageNpcStateId = ref(''); const brushMode = ref<'reveal' | 'hide'>('reveal'); const brushX = ref(.5); const brushY = ref(.5); const brushRadius = ref(.1); const imageUrl = ref(''); const error = ref(''); const busy = ref(false);
        ...
        const selectMap = (event: Event): void => setMap((event.target as HTMLSelectElement).value || null);
        const brush = async (): Promise<void> => { const session = selectedSession(); const map = selectedMap(); if (!session || !map || !progress.value) return; busy.value = true; error.value = ''; try { progress.value = (await api<ApiResponse<MapProgress>>(`/api/control/v1/campaigns/${campaignId}/sessions/${session.id}/maps/${map.id}/progress/fog`, { method: 'POST', body: JSON.stringify({ command_id: commandId(), expected_revision: progress.value.revision, mode: brushMode.value, center_x: brushX.value, center_y: brushY.value, radius: brushRadius.value }) })).data; } catch (reason) { error.value = reason instanceof Error ? reason.message : 'Unable to apply this fog brush.'; await loadWorkspace(); } finally { busy.value = false; } };
        ...
        const savePresentationEntries = async (entries: PresentationStateEntry[], presetId = presentation.value?.state.stage_preset_id ?? null, backdropId = presentation.value?.state.backdrop_asset_id ?? null, musicCueId = presentation.value?.state.music_cue_id ?? null): Promise<void> => { const session = selectedSession(); if (!session || !presentation.value) return; const state = presentation.value.state; busy.value = true; try { presentation.value = (await api<ApiResponse<PresentationSnapshot>>(`/api/control/v1/campaigns/${campaignId}/sessions/${session.id}/presentation-state`, { method: 'PUT', body: JSON.stringify({ command_id: commandId(), expected_revision: presentation.value.revision, state: { scene_id: state.scene_id, backdrop_asset_id: backdropId, music_cue_id: musicCueId, video_cue_id: state.video_cue_id, stage_preset_id: presetId, stage_entries: entries } }) })).data; stagePresetId.value = presetId ?? ''; await loadPresentationAssets(); } catch (reason) { error.value = reason instanceof Error ? reason.message : 'Unable to update staged NPCs.'; await loadWorkspace(); } finally { busy.value = false; } };
        ...
        const selectMusic = (event: Event): void => { if (presentation.value) void savePresentationEntries(presentation.value.state.stage_entries, presentation.value.state.stage_preset_id, presentation.value.state.backdrop_asset_id, (event.target as HTMLSelectElement).value || null); };
        onMounted(load); return { sessions, revisions, maps, scenes, audioCues, npcs, presets, selectedSessionId, createRevisionId, progressMode, playerMap, progress, presentation, presentationAssetUrls, activeEntries, activeScene, activeBackdrops, selectableNpcStates, standbySceneId, stagePresetId, stageNpcId, stageNpcStateId, brushMode, brushX, brushY, brushRadius, imageUrl, error, busy, selectedMap, loadWorkspace, create, setMap, selectMap, brush, brushAt, reset, saveTokens, moveTokens, standby, go, movePresentationEntry, addPresentationNpc, removePresentationEntry, applyStagePreset, resetSceneStage, clearPresentationStage, setBackdrop, selectBackdrop, selectMusic, back: () => router.push('/') };
    },
