...
type PresentationCue = { scene_id: string | null; backdrop_asset_id: string | null; music_cue_id: string | null; video_cue_id: string | null; stage_entries: PresentationStateEntry[] };
...
const SessionsView = defineComponent({
    ...
    setup() {
        ...
        const sessions = ref<LiveSessionRecord[]>([]); const revisions = ref<CampaignRevision[]>([]); const maps = ref<PinnedMap[]>([]); const scenes = ref<PinnedScene[]>([]); const npcs = ref<PinnedNpc[]>([]); const npcStates = ref<PinnedNpcState[]>([]); const presetEntries = ref<PinnedStagePresetEntry[]>([]); const selectedSessionId = ref(''); const createRevisionId = ref(''); const progressMode = ref<'fresh' | 'resume'>('fresh'); const playerMap = ref<PlayerMapState | null>(null); const progress = ref<MapProgress | null>(null); const presentation = ref<PresentationSnapshot | null>(null); const presentationAssetUrls = ref<Record<string, string>>({}); const standbySceneId = ref(''); const stageNpcId = ref(''); const stageNpcStateId = ref(''); const brushMode = ref<'reveal' | 'hide'>('reveal'); const brushX = ref(.5); const brushY = ref(.5); const brushRadius = ref(.1); const imageUrl = ref(''); const error = ref(''); const busy = ref(false);
        ...
        const savePresentationEntries = async (entries: PresentationStateEntry[]): Promise<void> => { const session = selectedSession(); if (!session || !presentation.value) return; const state = presentation.value.state; busy.value = true; try { presentation.value = (await api<ApiResponse<PresentationSnapshot>>(`/api/control/v1/campaigns/${campaignId}/sessions/${session.id}/presentation-state`, { method: 'PUT', body: JSON.stringify({ command_id: commandId(), expected_revision: presentation.value.revision, state: { scene_id: state.scene_id, backdrop_asset_id: state.backdrop_asset_id, music_cue_id: state.music_cue_id, video_cue_id: state.video_cue_id, stage_entries: entries } }) })).data; } catch (reason) { error.value = reason instanceof Error ? reason.message : 'Unable to update staged NPCs.'; await loadWorkspace(); } finally { busy.value = false; } };
