...
type PinnedMap = { id: string; name: string };
...
const SessionsView = defineComponent({
    setup() {
        ...
        const loadWorkspace = async (): Promise<void> => { const session = selectedSession(); if (!session) { maps.value = []; playerMap.value = null; progress.value = null; return; } const [revision, state] = await Promise.all([api<ApiResponse<{ manifest: { maps?: PinnedMap[] } }>>(`/api/control/v1/campaigns/${campaignId}/revisions/${session.campaign_revision_id}`), api<ApiResponse<PlayerMapState>>(`/api/control/v1/campaigns/${campaignId}/sessions/${session.id}/player-map`)]); maps.value = revision.data.manifest.maps ?? []; playerMap.value = state.data; progress.value = state.data.map_id ? (await api<ApiResponse<MapProgress>>(`/api/control/v1/campaigns/${campaignId}/sessions/${session.id}/maps/${state.data.map_id}/progress`)).data : null; };
