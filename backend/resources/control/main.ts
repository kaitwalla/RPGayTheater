import { computed, createApp, defineComponent, onBeforeUnmount, onMounted, ref, type PropType, watch } from 'vue';
...
type LivePickerOption = { value: string | number; label: string; disabled?: boolean };

const LivePicker = defineComponent({
    props: {
        modelValue: { type: [String, Number] as PropType<string | number>, required: true },
        options: { type: Array as PropType<LivePickerOption[]>, required: true },
    ...
    setup(props, { emit }) {
        ...
        const choose = (option: LivePickerOption): void => {
        ...
        const closeWhenBlurred = (event: PointerEvent): void => {
            if (!root.value?.contains(event.target as Node)) open.value = false;
        };

        onMounted(() => document.addEventListener('pointerdown', closeWhenBlurred));
        onBeforeUnmount(() => document.removeEventListener('pointerdown', closeWhenBlurred));

        return { root, open, selected, choose, toggle, keydown };
    },
    template: `<div ref="root" class="live-picker" @keydown="keydown"><button class="live-picker-trigger" type="button" :aria-label="label" aria-haspopup="listbox" :aria-expanded="open" :disabled="disabled" @click="toggle"><span>{{ selected?.label || label }}</span><span class="live-picker-chevron" aria-hidden="true"></span></button><div v-if="open" class="live-picker-options" role="listbox" :aria-label="label"><button v-for="option in options" :key="option.value" class="live-picker-option" :class="{ selected: option.value === modelValue }" type="button" role="option" :aria-selected="option.value === modelValue" :disabled="option.disabled" @click="choose(option)">{{ option.label }}</button></div></div>`,
});

const SessionsView = defineComponent({
    components: { ControlMapStage, DiceRollVisual, LivePicker, PresentationStage },
    setup() {
        const route = useRoute();
        const router = useRouter();
        const campaignId = String(route.params.campaign);
        ...
        const participantsRealtime = useRealtimeSnapshot<SessionParticipantRecord[]>({
            load: async () => {
                await loadParticipants();
            ...
            channel: () => {
                ...
                return session ? `session_participants.${session.id}` : [];
            },
        });
        ...
        onMounted(async () => {
            ...
            await participantsRealtime.start();
            await loadPlayerGroups();
            await loadMessages();
            await loadRolls();
            await loadNpcReveals();
    ...
    template: `<main class="control-workspace">
        ...
        <div class="control-grid">
            <section class="control-main stack" :class="{ 'tools-collapsed': toolsCollapsed, 'presentation-preview-hidden': activeLiveTab === 'presentation' && !showControlPreview }" aria-label="Live workspace">
                ...
                <section v-if="activeLiveTab === 'presentation' && presentation" class="control-stage-card presentation-stage-card stack">
                    <header class="control-section-header"><div><h2>Presentation</h2><p class="muted">Choose a scene, then show or hide this Control-only preview.</p></div><div class="row"><button class="secondary" :aria-pressed="showControlPreview" @click="showControlPreview = !showControlPreview">{{ showControlPreview ? 'Hide preview' : 'Show preview' }}</button><button :disabled="busy || !presentationDirty" @click="updatePresentation">{{ busy ? 'Updating…' : 'Update' }}</button></div></header>
                ...
                <section v-else-if="activeLiveTab === 'map' && progress && selectedMap()" class="control-stage-card stack">
                    ...
                    <div class="control-form-grid"><LivePicker v-model="mapInteraction" :options="[{ value: 'tokens', label: 'Move tokens' }, { value: 'fog', label: 'Paint fog' }]" label="Map editing mode" :disabled="busy"/><LivePicker v-model="brushMode" :options="[{ value: 'reveal', label: 'Reveal fog' }, { value: 'hide', label: 'Hide with fog' }]" label="Fog brush mode" :disabled="busy"/><label>X <input v-model.number="brushX" type="number" min="0" max="1" step=".01"></label><label>Y <input v-model.number="brushY" type="number" min="0" max="1" step=".01"></label><label>Radius <input v-model.number="brushRadius" type="number" min=".005" max="1" step=".01"></label><button :disabled="busy" @click="brush">Apply brush</button></div>
            ...
            <aside class="control-sidebar stack" aria-label="Live controls">
                <section v-if="presentation" class="control-card stack compact"><h2>Stage backdrop + preset</h2><LivePicker :model-value="currentPresentationCue()?.backdrop_asset_id || ''" :options="[{ value: '', label: 'No backdrop' }, ...activeBackdrops.map((backdrop) => ({ value: backdrop.asset_id, label: backdrop.name }))]" label="Scene backdrop" :disabled="busy" @update:model-value="setBackdrop"/><button class="secondary" :disabled="busy || !activeScene" @click="setBackdrop(activeScene.primary_backdrop_asset_id || '')">Use primary backdrop</button><LivePicker v-model="stagePresetId" :options="[{ value: '', label: 'Empty stage' }, ...activeScenePresets.map((preset) => ({ value: preset.id, label: preset.name }))]" label="Stage preset" :disabled="busy || !activeScene"/><div class="button-grid"><button :disabled="busy || !activeScene" @click="applyStagePreset">Apply preset</button><button class="secondary" :disabled="busy || !activeScene" @click="resetSceneStage">Reset scene</button><button class="danger" :disabled="busy" @click="clearPresentationStage">Clear stage</button></div></section>
                <section v-if="presentation" class="control-card stack compact emotion-control"><header class="emotion-control-header"><div><h2>Character emotions</h2><p v-if="activeEntries.length" class="muted">Select characters, then change their shared emotion.</p></div><span v-if="activeEntries.length" class="status-pill">{{ selectedStageEntries.length }} selected</span></header><template v-if="activeEntries.length"><div class="emotion-action-row"><LivePicker v-model="bulkStageEmotion" :options="[{ value: '', label: 'Choose emotion' }, { value: '__normal', label: 'Normal' }, ...bulkStageEmotionOptions.map((name) => ({ value: name, label: name }))]" label="Selected character emotion" :disabled="busy || selectedStageEntries.length === 0"/><button class="secondary" :disabled="busy || selectedStageEntries.length === 0 || !bulkStageEmotion" @click="applySelectedStageEmotion">Apply</button></div><div class="emotion-target-list"><label v-for="entry in activeEntries" :key="'emotion:' + stageEntryKey(entry)" class="emotion-target" :class="{ selected: selectedStageEntryKeys.includes(stageEntryKey(entry)) }"><input v-model="selectedStageEntryKeys" type="checkbox" :value="stageEntryKey(entry)" :disabled="busy"><span>{{ entry.name }}</span><small>{{ entry.npc_state_id ? (npcStates.find((state) => state.id === entry.npc_state_id)?.name || 'Emotion') : 'Normal' }}</small></label></div></template><p v-else class="muted">Stage a character to control its emotion.</p></section>
                <section v-if="presentation" class="control-card stack compact"><h2>Stage composition</h2><LivePicker v-model="stageNpcId" :options="[{ value: '', label: 'Choose a character' }, ...npcs.map((npc) => ({ value: npc.id, label: npc.name }))]" label="NPC to stage" :disabled="busy"/><LivePicker v-model="stageNpcScale" :options="stageScaleOptions.map((scale) => ({ value: scale, label: scale + 'x' }))" label="NPC session size" :disabled="busy"/><button :disabled="busy || !stageNpcId" @click="addPresentationNpc">Stage character</button><template v-if="activeEntries.length"><article v-for="entry in activeEntries" :key="'placement:' + stageEntryKey(entry)" class="compact-asset"><span>{{ entry.name }} · {{ entry.scale }}x · L{{ entry.layer_order + 1 }}</span><div class="row"><button class="secondary" :aria-pressed="entry.facing === 'left'" :disabled="busy" @click="setPresentationEntryFacing(entry, 'left')">Face left</button><button class="secondary" :aria-pressed="entry.facing !== 'left'" :disabled="busy" @click="setPresentationEntryFacing(entry, 'right')">Face right</button><button class="danger" :disabled="busy" @click="removePresentationEntry(entry)">Remove</button></div></article></template></section>
                <section v-if="playerMap" class="control-card stack compact">
                    <h2>Player map</h2>
                    <LivePicker :model-value="playerMap.map_id || ''" :options="[{ value: '', label: 'Hide Player map' }, ...maps.map((map) => ({ value: map.id, label: map.name }))]" label="Current Player map" :disabled="busy" @update:model-value="(mapId) => setMap(mapId || null)"/>
