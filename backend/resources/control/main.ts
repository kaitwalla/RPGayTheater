...
const SessionsView = defineComponent({
    ...
    template: `<main class="shell stack">
        ...
        <p v-if="error" class="error" role="alert">{{ error }}</p>
        <section v-if="presentation" class="panel stack"><h2>Scene music</h2><select :value="presentation.state.music_cue_id || ''" aria-label="Scene music" @change="selectMusic"><option value="">Stop music</option><option v-for="cue in audioCues.filter((cue) => cue.kind === 'music')" :key="cue.id" :value="cue.id">{{ cue.name }}</option></select><template v-if="presentation.state.music_cue_id"><div class="row"><button class="secondary" :disabled="busy" @click="saveMusicPlayback({ status: 'playing' })">Play</button><button class="secondary" :disabled="busy" @click="saveMusicPlayback({ status: 'paused' })">Pause</button><button class="secondary" :disabled="busy" @click="seekMusic(0)">Restart</button><button class="danger" :disabled="busy" @click="saveMusicPlayback({ status: 'stopped', position_seconds: 0 })">Stop</button></div><div class="row"><label>Seek (seconds) <input :value="presentation.state.music_playback.position_seconds" type="number" min="0" step=".1" @change="setMusicPosition"></label><label>Volume (%) <input :value="Math.round(presentation.state.music_playback.volume * 100)" type="number" min="0" max="100" @change="setMusicVolume"></label><label>Fade (ms) <input :value="presentation.state.music_playback.fade_duration_ms" type="number" min="0" max="30000" step="100" @change="setMusicFade"></label><label><input :checked="presentation.state.music_playback.loop" type="checkbox" @change="setMusicLoop"> Loop</label></div></template></section>
        <section v-if="presentation" class="panel stack"><header class="row"><h2>Sound effects</h2><button class="danger" :disabled="busy || !(presentation.state.sfx_instances || []).length" @click="stopAllSfx">Stop all</button></header><div class="row"><label>Master volume (%) <input :value="Math.round((presentation.state.sfx_master_volume ?? 1) * 100)" type="number" min="0" max="100" @change="setSfxMasterVolume"></label></div><div class="row"><button v-for="cue in audioCues.filter((cue) => cue.kind === 'sfx')" :key="cue.id" :disabled="busy" @click="triggerSfx(cue.id)">Play {{ cue.name }}</button></div><p v-if="audioCues.filter((cue) => cue.kind === 'sfx').length === 0" class="muted">No sound effects are pinned to this session.</p><article v-for="instance in presentation.state.sfx_instances || []" :key="instance.id" class="asset"><div><strong>{{ audioCues.find((cue) => cue.id === instance.cue_id)?.name || 'Sound effect' }}</strong><div class="muted">{{ instance.loop ? 'Looping' : 'One shot' }} · {{ Math.round(instance.volume * 100) }}%</div></div><button class="danger" :disabled="busy" @click="stopSfx(instance.id)">Stop</button></article></section>
        <section v-if="presentation" class="panel stack"><h2>Fullscreen video</h2><p class="muted">Starting a cue captures the current scene. Abort restores that captured scene.</p><select :value="presentation.state.video_cue_id || ''" aria-label="Fullscreen video" @change="selectVideo"><option value="">No active video</option><option v-for="cue in videoCues" :key="cue.id" :value="cue.id">{{ cue.name }} · {{ cue.completion_mode }} · music {{ cue.music_during }}/{{ cue.music_after }}</option></select><button v-if="presentation.state.video_cue_id" class="danger" :disabled="busy" @click="abortVideo">Abort video and restore scene</button></section>
        <section class="panel stack"><h2>Start a session</h2><select v-model="createRevisionId" aria-label="Published revision"><option value="">Choose a published revision</option><option v-for="revision in revisions" :key="revision.id" :value="revision.id">Revision {{ revision.number }}</option></select><select :value="progressMode" aria-label="Progress mode" @change="setProgressMode"><option value="fresh">Fresh progress</option><option value="resume">Resume progress</option></select><label><input v-model="copyPlayerGroups" type="checkbox"> Copy named Player groups from the previous session</label><button :disabled="busy || !createRevisionId" @click="create">Start session</button></section>
        <section class="panel stack"><h2>Active session</h2><select v-model="selectedSessionId" aria-label="Live session" @change="selectSession"><option value="">Choose session</option><option v-for="session in sessions" :key="session.id" :value="session.id">{{ session.player_code }} · {{ session.status }}</option></select><template v-if="playerMap"><div class="row"><select :value="playerMap.map_id || ''" aria-label="Current Player map" @change="selectMap"><option value="">Hide Player map</option><option v-for="map in maps" :key="map.id" :value="map.id">{{ map.name }}</option></select><button class="secondary" :disabled="busy" @click="setMap(null)">Hide map</button></div><p v-if="!playerMap.map_id" class="muted">No map is currently shared with Players or Spectators.</p></template></section>
        <section v-if="selectedSessionId" class="panel stack"><h2>Adopt published revision</h2><p class="muted">Preflight protects active claims, presentation state, Player maps, fog, and token references before switching this live session.</p><select v-model="adoptionRevisionId" aria-label="Revision to adopt" @change="adoptionPreflight = null"><option value="">Choose a published revision</option><option v-for="revision in revisions.filter((item) => item.id !== selectedSession()?.campaign_revision_id)" :key="revision.id" :value="revision.id">Revision {{ revision.number }}</option></select><div class="row"><button class="secondary" :disabled="busy || !adoptionRevisionId" @click="preflightRevisionAdoption">Check compatibility</button><button :disabled="busy || !adoptionPreflight?.compatible" @click="adoptRevision">Adopt revision</button></div><template v-if="adoptionPreflight"><p :class="adoptionPreflight.compatible ? 'muted' : 'error'">{{ adoptionPreflight.compatible ? 'This revision is compatible with the current live state.' : 'This revision cannot preserve the current live state.' }}</p><p v-if="adoptionPreflight.compatible" class="muted">{{ Object.entries(adoptionPreflight.changes).filter(([, change]) => change.added.length || change.removed.length || change.changed.length).length }} authored collections change.</p><ul v-else><li v-for="blocker in adoptionPreflight.blockers" :key="blocker.type + ':' + (blocker.reference_id || blocker.player_character_id || blocker.map_id || '')">{{ blocker.type.replaceAll('_', ' ') }}{{ blocker.reference_type ? ' · ' + blocker.reference_type : '' }}</li></ul><article v-for="(change, collection) in adoptionPreflight.changes" :key="collection" v-show="change.added.length || change.removed.length || change.changed.length" class="asset"><strong>{{ collection.replaceAll('_', ' ') }}</strong><div class="muted">{{ changeSummary(change) }}</div></article></template></section>
        <section v-if="selectedSessionId" class="panel stack"><h2>Participants</h2><p v-if="participants.length === 0" class="muted">No participants have joined this session.</p><article v-for="participant in participants" :key="participant.id" class="asset"><div><strong>{{ participant.display_name }}</strong><div class="muted">{{ participant.role }}{{ participant.player_character_id ? ' · character claimed' : '' }}{{ participant.revoked_at ? ' · revoked' : '' }}</div></div><div class="row"><button v-if="participant.player_character_id && !participant.revoked_at" class="secondary" :disabled="busy" @click="releaseClaim(participant)">Release claim</button><button v-if="!participant.revoked_at" class="danger" :disabled="busy" @click="revokeParticipant(participant)">Revoke</button></div></article></section>
        <section v-if="selectedSessionId" class="panel stack"><h2>Player groups</h2><p class="muted">Groups are session-scoped. Resumed groups restore a member only when that Player claims the same character.</p><div class="row"><input v-model="playerGroupName" maxlength="120" aria-label="Player group name" placeholder="Group name"><button :disabled="busy || !playerGroupName.trim()" @click="createPlayerGroup">Create group</button></div><p v-if="playerGroups.length === 0" class="muted">No Player groups yet.</p><article v-for="group in playerGroups" :key="group.id" class="asset"><div><strong>{{ group.name }}</strong><div class="muted">{{ group.member_participant_ids.length }} member{{ group.member_participant_ids.length === 1 ? '' : 's' }}</div></div><div class="stack"><label v-for="participant in participants.filter((item) => item.role === 'player' && !item.revoked_at)" :key="participant.id"><input :checked="group.member_participant_ids.includes(participant.id)" type="checkbox" :disabled="busy" @change="setPlayerGroupMember(group, participant, $event)"> {{ participant.display_name }}</label></div></article></section>
        <section v-if="selectedSessionId" class="panel stack"><h2>Polls</h2><form class="stack" @submit.prevent="createPoll"><input v-model="pollQuestion" maxlength="500" aria-label="Poll question" placeholder="Poll question"><textarea v-model="pollOptions" maxlength="6000" aria-label="Poll options" placeholder="One option per line"></textarea><select v-model="pollAudience" aria-label="Poll audience"><option value="all">All participants</option><option value="all_players">All Players</option><option value="all_spectators">All Spectators</option><option value="individual">Individual participant</option><option value="player_group">Named Player group</option></select><select v-if="pollAudience === 'individual'" v-model="pollParticipantId" aria-label="Poll participant"><option value="">Choose participant</option><option v-for="participant in participants.filter((item) => !item.revoked_at)" :key="participant.id" :value="participant.id">{{ participant.display_name }} · {{ participant.role }}</option></select><select v-if="pollAudience === 'player_group'" v-model="pollGroupId" aria-label="Poll Player group"><option value="">Choose Player group</option><option v-for="group in playerGroups" :key="group.id" :value="group.id">{{ group.name }}</option></select><label><input v-model="pollMultiple" type="checkbox"> Allow multiple choices</label><button :disabled="busy || !pollQuestion.trim() || pollOptions.split('\\n').filter((option) => option.trim()).length < 2 || (pollAudience === 'individual' && !pollParticipantId) || (pollAudience === 'player_group' && !pollGroupId)">Create poll</button></form><p v-if="polls.length === 0" class="muted">No polls yet.</p><article v-for="poll in polls" :key="poll.id" class="asset"><div><strong>{{ poll.question }}</strong><div class="muted">{{ poll.status }} · results {{ poll.result_visibility }}</div><div v-for="option in poll.options" :key="option.id">{{ option.body }} · {{ option.votes }}</div></div><div class="row"><button v-if="poll.status === 'open'" class="danger" :disabled="busy" @click="pollAction(poll, 'close')">Close</button><button class="secondary" :disabled="busy" @click="pollAction(poll, 'live')">Publish live</button><button v-if="poll.status === 'closed'" class="secondary" :disabled="busy" @click="pollAction(poll, 'final')">Publish final</button></div></article></section>
        <section v-if="selectedSessionId" class="panel stack"><h2>Rolls</h2><p v-if="sessionRolls.length === 0" class="muted">No rolls yet.</p><article v-for="roll in sessionRolls" :key="roll.id" class="asset"><div><strong>{{ roll.roller_name }} rolled {{ roll.total }}</strong><div class="muted">{{ roll.expression }} · {{ roll.visibility }}{{ roll.dice_preset_name ? ' · ' + roll.dice_preset_name : '' }}</div></div><button v-if="roll.visibility === 'private'" class="secondary" :disabled="busy" @click="revealRoll(roll)">Reveal publicly</button></article></section>
        <section v-if="selectedSessionId" class="panel stack"><h2>Messages</h2><p class="muted">Broad messages snapshot their current audience. Replies to them return privately to Control; selected Spectator replies can appear on Presentation.</p><p v-if="sessionMessages.length === 0" class="muted">No messages yet.</p><article v-for="message in sessionMessages" :key="message.id" class="asset"><div><strong>{{ message.sender_name }}</strong><div>{{ message.body }}</div><div class="muted">{{ message.target_type.replaceAll('_', ' ') }} · {{ new Date(message.created_at).toLocaleTimeString() }}</div></div><button v-if="canPublishSpectatorReply(message)" class="secondary" :disabled="busy" @click="publishSpectatorReply(message)">Publish to Presentation</button></article><form class="stack" @submit.prevent="sendMessage"><h3>Send message</h3><select v-model="messageTargetType" aria-label="Message audience"><option value="all">All participants</option><option value="all_players">All Players</option><option value="all_spectators">All Spectators</option><option value="individual">Individual participant</option><option value="player_group">Named Player group</option></select><select v-if="messageTargetType === 'individual'" v-model="messageParticipantId" aria-label="Individual participant"><option value="">Choose participant</option><option v-for="participant in participants.filter((item) => !item.revoked_at)" :key="participant.id" :value="participant.id">{{ participant.display_name }} · {{ participant.role }}</option></select><select v-if="messageTargetType === 'player_group'" v-model="messageGroupId" aria-label="Named Player group"><option value="">Choose Player group</option><option v-for="group in playerGroups" :key="group.id" :value="group.id">{{ group.name }}</option></select><textarea v-model="messageBody" maxlength="2000" aria-label="Plain-text message" placeholder="Plain-text message"></textarea><button :disabled="busy || !messageBody.trim() || (messageTargetType === 'individual' && !messageParticipantId) || (messageTargetType === 'player_group' && !messageGroupId)">Send</button></form></section>
        <section v-if="selectedSessionId" class="panel stack"><h2>NPC profiles</h2><p class="muted">Presentation staging never reveals a profile. Choose exactly what Players and Spectators can read.</p><p v-if="npcs.length === 0" class="muted">No NPCs are pinned to this session.</p><article v-for="npc in npcs" :key="npc.id" class="asset"><div><strong>{{ npc.name }}</strong><div class="muted">{{ npcIsRevealed(npc.id) ? 'Revealed to participants' : 'Hidden from participants' }}</div></div><button v-if="npcIsRevealed(npc.id)" class="danger" :disabled="busy" @click="setNpcReveal(npc.id, false)">Hide profile</button><button v-else :disabled="busy" @click="setNpcReveal(npc.id, true)">Reveal profile</button></article></section>
        <section v-if="selectedSessionId" class="panel stack"><h2>Shared NPC notes</h2><p class="muted">Control may moderate any note, including notes from archived sessions.</p><p v-if="npcNotes.length === 0" class="muted">No shared NPC notes yet.</p><article v-for="note in npcNotes" :key="note.id" class="asset"><div><strong>{{ npcs.find((npc) => npc.id === note.npc_id)?.name || 'NPC' }}</strong><div>{{ note.body }}</div><div class="muted">{{ note.author_type === 'control' ? 'Control' : participants.find((participant) => participant.id === note.session_participant_id)?.display_name || 'Player' }}</div></div><div class="row"><button class="secondary" :disabled="busy" @click="editNpcNote(note)">Edit</button><button class="danger" :disabled="busy" @click="deleteNpcNote(note)">Delete</button></div></article></section>
        <section v-if="presentation" class="panel stack"><h2>Presentation preview</h2><p class="muted">Drag a staged NPC to reposition it. Every edit saves as a revisioned active-stage update.</p><PresentationStage :backdrop-asset-id="presentation.state.backdrop_asset_id" :transition="activeScene?.transition || 'cut'" :transition-duration-ms="activeScene?.transition_duration_ms || 0" :stage-tween-duration-ms="presets.find((preset) => preset.id === presentation.state.stage_preset_id)?.tween_duration_ms || 0" :stage-tween-easing="presets.find((preset) => preset.id === presentation.state.stage_preset_id)?.tween_easing || 'linear'" :entries="activeEntries" :asset-urls="presentationAssetUrls" editable @move-entry="movePresentationEntry" /><div class="row"><select :value="presentation.state.backdrop_asset_id || ''" aria-label="Scene backdrop" @change="selectBackdrop"><option value="">No backdrop</option><option v-for="backdrop in activeBackdrops" :key="backdrop.id" :value="backdrop.asset_id">{{ backdrop.name }}</option></select><button class="secondary" :disabled="busy || !activeScene" @click="setBackdrop(activeScene.primary_backdrop_asset_id || '')">Use primary backdrop</button></div><div class="row"><button class="secondary" :disabled="busy || !activeScene" @click="resetSceneStage">Reset scene stage</button><button class="danger" :disabled="busy" @click="clearPresentationStage">Clear stage</button></div><div class="row"><select v-model="stagePresetId" aria-label="Stage preset"><option value="">Empty stage</option><option v-for="preset in presets" :key="preset.id" :value="preset.id">{{ preset.name }}</option></select><button :disabled="busy" @click="applyStagePreset">Apply preset</button></div><div class="row"><select v-model="stageNpcId" aria-label="NPC to stage" @change="stageNpcStateId = ''"><option value="">Add an NPC</option><option v-for="npc in npcs" :key="npc.id" :value="npc.id">{{ npc.name }}</option></select><select v-model="stageNpcStateId" aria-label="NPC state"><option value="">Normal appearance</option><option v-for="state in selectableNpcStates" :key="state.id" :value="state.id">{{ state.name }}</option></select><button :disabled="busy || !stageNpcId" @click="addPresentationNpc">Add to stage</button></div><article v-for="entry in activeEntries" :key="entry.npc_id + ':' + entry.npc_state_id + ':' + entry.layer_order" class="asset"><strong>{{ entry.name }}</strong><span class="muted">Layer {{ entry.layer_order + 1 }}</span><button class="danger" :disabled="busy" @click="removePresentationEntry(entry)">Remove</button></article></section>
        <section v-if="presentation" class="panel stack"><h2>Presentation standby</h2><p class="muted">Live scene: {{ presentation.state.scene_id || 'None' }} · Standby: {{ presentation.state.standby_status }}{{ presentation.state.standby_error ? ' — ' + presentation.state.standby_error : '' }}</p><div class="row"><select v-model="standbySceneId" aria-label="Standby scene"><option value="">Choose scene</option><option v-for="scene in scenes" :key="scene.id" :value="scene.id">{{ scene.name }}</option></select><button :disabled="busy || !standbySceneId" @click="standby">Standby</button><button :disabled="busy || presentation.state.standby_status !== 'ready'" @click="go">Go</button></div></section>
        <section v-if="progress && selectedMap()" class="panel stack"><header class="row"><div><h2>{{ selectedMap()?.name }}</h2><p class="muted">Map revision {{ progress.revision }} · {{ progress.fog.brushes.length }} live brush operations</p></div><button class="danger" :disabled="busy" @click="reset">Reset map</button></header><p class="muted">{{ mapInteraction === 'fog' ? 'Drag across the map to paint fog. Each stroke is applied in order.' : 'Select, marquee, and drag tokens. Switch to fog painting to reveal or hide areas.' }}</p><ControlMapStage :image-url="imageUrl" :tokens="progress.tokens" :fog="progress.fog" :brush-mode="brushMode" :brush-radius="brushRadius" :interaction-mode="mapInteraction" :disabled="busy" @brush-stroke="brushStroke" @move-tokens="moveTokens" /><div class="row"><select v-model="mapInteraction" aria-label="Map editing mode"><option value="tokens">Move tokens</option><option value="fog">Paint fog</option></select><select v-model="brushMode" aria-label="Fog brush mode"><option value="reveal">Reveal fog</option><option value="hide">Hide with fog</option></select><label>X <input v-model.number="brushX" type="number" min="0" max="1" step=".01"></label><label>Y <input v-model.number="brushY" type="number" min="0" max="1" step=".01"></label><label>Radius <input v-model.number="brushRadius" type="number" min=".005" max="1" step=".01"></label><button :disabled="busy" @click="brush">Apply brush</button></div><section class="stack"><h3>Token positions</h3><article v-for="token in progress.tokens" :key="token.source_token_id" class="asset"><strong>{{ token.label || token.source_token_id }}</strong><label>X <input v-model.number="token.position_x" type="number" min="0" max="1" step=".01"></label><label>Y <input v-model.number="token.position_y" type="number" min="0" max="1" step=".01"></label><label>Scale <input v-model.number="token.scale" type="number" min=".1" max="5" step=".1"></label></article><button :disabled="busy" @click="saveTokens">Save token positions</button></section></section>
        <section v-if="presentation && activeEntries.length" class="panel stack"><header class="row"><div><div class="eyebrow">Live expression controls</div><h2>Change emotions on stage</h2></div><span class="muted">Each change updates the live stage immediately.</span></header><article v-for="entry in activeEntries" :key="'emotion:' + entry.npc_id + ':' + entry.layer_order" class="asset"><div><strong>{{ entry.name }}</strong><div class="muted">Layer {{ entry.layer_order + 1 }}</div></div><select :value="entry.npc_state_id || ''" :aria-label="'Live emotion for ' + entry.name" :disabled="busy" @change="setPresentationEntryEmotion(entry, $event)"><option value="">Normal</option><option v-for="state in npcStates.filter((state) => state.npc_id === entry.npc_id)" :key="state.id" :value="state.id">{{ state.name }}</option></select></article></section>
    </main>`,
});
