...
const SessionsView = defineComponent({
    ...
    setup() {
        ...
        const sessions = ref<LiveSessionRecord[]>([]); const revisions = ref<CampaignRevision[]>([]); const maps = ref<PinnedMap[]>([]); const selectedSessionId = ref(''); const createRevisionId = ref(''); const progressMode = ref<'fresh' | 'resume'>('fresh'); const playerMap = ref<PlayerMapState | null>(null); const progress = ref<MapProgress | null>(null); const brushMode = ref<'reveal' | 'hide'>('reveal'); const brushX = ref(.5); const brushY = ref(.5); const brushRadius = ref(.1); const imageUrl = ref(''); const error = ref(''); const busy = ref(false);
        ...
        const loadWorkspace = async (): Promise<void> => { const session = selectedSession(); if (!session) { maps.value = []; playerMap.value = null; progress.value = null; imageUrl.value = ''; return; } const [revision, state] = await Promise.all([api<ApiResponse<{ manifest: { maps?: PinnedMap[] } }>>(`/api/control/v1/campaigns/${campaignId}/revisions/${session.campaign_revision_id}`), api<ApiResponse<PlayerMapState>>(`/api/control/v1/campaigns/${campaignId}/sessions/${session.id}/player-map`)]); maps.value = revision.data.manifest.maps ?? []; playerMap.value = state.data; const map = selectedMap(); progress.value = state.data.map_id ? (await api<ApiResponse<MapProgress>>(`/api/control/v1/campaigns/${campaignId}/sessions/${session.id}/maps/${state.data.map_id}/progress`)).data : null; imageUrl.value = map ? (await api<ApiResponse<{ url: string }>>(`/api/control/v1/campaigns/${campaignId}/assets/${map.image_asset_id}/read`)).data.url : ''; };
        ...
        onMounted(load); return { sessions, revisions, maps, selectedSessionId, createRevisionId, progressMode, playerMap, progress, brushMode, brushX, brushY, brushRadius, imageUrl, error, busy, selectedMap, loadWorkspace, create, setMap, selectMap, brush, brushAt, reset, saveTokens, moveTokens, back: () => router.push('/') };
    },
    template: `<main class="shell stack"><header class="row"><div><div class="eyebrow">Live session</div><h1>Maps and fog</h1></div><button class="secondary" @click="back">Campaigns</button></header><p v-if="error" class="error" role="alert">{{ error }}</p><section class="panel stack"><h2>Start a session</h2><select v-model="createRevisionId" aria-label="Published revision"><option value="">Choose a published revision</option><option v-for="revision in revisions" :key="revision.id" :value="revision.id">Revision {{ revision.number }}</option></select><select v-model="progressMode" aria-label="Progress mode"><option value="fresh">Fresh progress</option><option value="resume">Resume progress</option></select><button :disabled="busy || !createRevisionId" @click="create">Start session</button></section><section class="panel stack"><h2>Active session</h2><select v-model="selectedSessionId" aria-label="Live session" @change="loadWorkspace"><option value="">Choose session</option><option v-for="session in sessions" :key="session.id" :value="session.id">{{ session.player_code }} · {{ session.status }}</option></select><p v-if="sessions.length === 0" class="muted">Start a session from a published revision to control its Player map.</p><template v-if="playerMap"><div class="row"><select :value="playerMap.map_id || ''" aria-label="Current Player map" @change="selectMap"><option value="">Hide Player map</option><option v-for="map in maps" :key="map.id" :value="map.id">{{ map.name }}</option></select><button class="secondary" :disabled="busy" @click="setMap(null)">Hide map</button></div><p v-if="!playerMap.map_id" class="muted">No map is currently shared with Players or Spectators.</p></template></section><section v-if="progress && selectedMap()" class="panel stack"><header class="row"><div><h2>{{ selectedMap()?.name }}</h2><p class="muted">Map revision {{ progress.revision }} · {{ progress.fog.brushes.length }} live brush operations</p></div><button class="danger" :disabled="busy" @click="reset">Reset map</button></header><ControlMapStage :image-url="imageUrl" :tokens="progress.tokens" :brush-mode="brushMode" :brush-radius="brushRadius" @brush="brushAt" @move-tokens="moveTokens" /><div class="row"><select v-model="brushMode" aria-label="Fog brush mode"><option value="reveal">Reveal fog</option><option value="hide">Hide with fog</option></select><label>X <input v-model.number="brushX" type="number" min="0" max="1" step=".01"></label><label>Y <input v-model.number="brushY" type="number" min="0" max="1" step=".01"></label><label>Radius <input v-model.number="brushRadius" type="number" min=".005" max="1" step=".01"></label><button :disabled="busy" @click="brush">Apply brush</button></div><section class="stack"><h3>Token positions</h3><article v-for="token in progress.tokens" :key="token.source_token_id" class="asset"><strong>{{ token.label || token.source_token_id }}</strong><label>X <input v-model.number="token.position_x" type="number" min="0" max="1" step=".01"></label><label>Y <input v-model.number="token.position_y" type="number" min="0" max="1" step=".01"></label><label>Scale <input v-model.number="token.scale" type="number" min=".1" max="5" step=".1"></label></article><button :disabled="busy" @click="saveTokens">Save token positions</button></section></section></main>`,
});
