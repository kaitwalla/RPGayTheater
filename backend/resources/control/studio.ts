        ...
        <main v-if="studio" class="studio-shell">
            ...
            <section class="studio-main"><header class="studio-header"><div><div class="eyebrow">Campaign studio</div><h1>{{ studio.campaign.name }}</h1></div><div class="row"><button class="secondary" :disabled="history.length === 0 || saving === 'saving'" @click="undo">Undo</button><button class="secondary" :disabled="redoHistory.length === 0 || saving === 'saving'" @click="redo">Redo</button><button @click="active = 'publish'">Review & publish</button></div></header>
                ...
                <section v-if="active === 'cast'" class="studio-content cast-studio stack">
                    ...
                    <section v-if="records('player_characters').length + records('npcs').length" class="cue-shelf stack">
                        <header><div><div class="eyebrow">Private reference</div><h3>Control-only character notes</h3><p class="muted">These notes stay in Control and are never published to the live display or participants.</p></div></header>
                        <label v-for="record in [...records('player_characters'), ...records('npcs')]" :key="'notes:' + record.id">{{ title(record) }}<textarea :value="record.control_notes || ''" :aria-label="'Control-only notes for ' + title(record)" maxlength="10000" placeholder="Secrets, motivations, and other GM-only context." @input="record.control_notes = textareaValue($event) || null; queueWrite(isNpc(record) ? 'npcs' : 'player-characters', record, ['control_notes'])"></textarea></label>
                    </section>
