...
type SceneModal = 'character' | 'backdrop' | 'stage' | 'stage-entry' | null;
...
type SceneStageForm = { name: string; tweenDurationMs: number; tweenEasing: 'linear' | 'ease_in' | 'ease_out' | 'ease_in_out' };
...
export const CampaignStudioView = defineComponent({
    setup() {
        ...
        const sceneStageForm = ref<SceneStageForm>({ name: '', tweenDurationMs: 800, tweenEasing: 'ease_in_out' });
        ...
        const openSceneModal = (modal: Exclude<SceneModal, null>): void => {
            if (modal === 'stage') sceneStageForm.value.name = selectedScene.value ? `${title(selectedScene.value)} stage` : 'Room stage';
        ...
        const createSceneStage = async (): Promise<string | null> => {
            if (!studio.value || !selectedScene.value || !sceneStageForm.value.name.trim()) return null;
            const response = await api<ApiResponse<StudioRecord>>(`/api/control/v1/campaigns/${campaignId}/stage-presets`, {
                method: 'POST',
                body: JSON.stringify({ command_id: commandId(), expected_revision: studio.value.campaign.draft_revision, name: sceneStageForm.value.name, tween_duration_ms: sceneStageForm.value.tweenDurationMs, tween_easing: sceneStageForm.value.tweenEasing }),
            });
            await load();
        ...
        const ensureSceneStage = async (): Promise<string | null> => {
            ...
            sceneStageForm.value.name = `${title(selectedScene.value)} stage`;
            return createSceneStage();
        };

        const submitSceneStage = async (): Promise<void> => {
            if (!sceneStageForm.value.name.trim()) return;
            busy.value = true; error.value = '';
            try {
                await createSceneStage();
                closeSceneModal();
            } catch (reason) { error.value = reason instanceof Error ? reason.message : 'Unable to create this room stage.'; await load(); }
            finally { busy.value = false; }
        };
        ...
        onMounted(load);
        return { sections: studioSections, active, assets, readyImages, readyAudio, readyVideos, stageEntries, selectedSceneId, selectedScene, mapTokens, selectedMap, selectedFog, filteredCueLibrary, studio, saving, error, busy, history, redoHistory, stagePresetId, mapId, fogAssetId, sceneCueForms, sceneAttachCueIds, sceneModal, sceneCharacterForm, sceneBackdropForm, sceneStageForm, sceneStageEntryForm, cueSearch, cueScopeFilter, cueTypeFilter, cueSceneFilter, assetUrls, collectionName, summary, records, title, inputValue, inputChecked, selectValue, textareaValue, cueType, cueResource, cueScope, sceneCues, sceneBackdrops, npcStates, attachableCues, sceneCueForm, openSceneModal, closeSceneModal, selectScene, loadAssetUrl, queueWrite, undo, redo, addCollection, updateCollectionMembership, beginDrag, setFog, attachCueToScene, makeCueGlobal, createSceneAudio, createSceneVideo, submitSceneCharacter, submitSceneBackdrop, submitSceneStage, submitSceneStageEntry, remove, publish, back: () => router.push('/'), openLegacy: (section: string) => router.push(`/campaigns/${campaignId}/${section}`), openSessions: () => router.push(`/campaigns/${campaignId}/sessions`) };
    },
    template: `
        <main v-if="studio" class="studio-shell">
            ...
            <section class="studio-main"><header class="studio-header"><div><div class="eyebrow">Campaign studio</div><h1>{{ studio.campaign.name }}</h1></div><div class="row"><button class="secondary" :disabled="history.length === 0 || saving === 'saving'" @click="undo">Undo</button><button class="secondary" :disabled="redoHistory.length === 0 || saving === 'saving'" @click="redo">Redo</button><button @click="active = 'publish'">Review & publish</button></div></header>
                ...
                <section v-if="active === 'scenes'" class="studio-content stack"><header class="section-heading"><div><div class="eyebrow">Location-first authoring</div><h2>Scenes & rooms</h2><p class="muted">Pick a room, then edit the media, cues, and staging it needs whenever the party reaches it.</p></div><div class="row"><button class="secondary" @click="openLegacy('scenes')">Add room</button><button @click="openSceneModal('stage')">New room stage</button></div></header><div class="scene-workspace"><aside class="scene-selector stack"><div class="row"><h3>Rooms</h3><span class="muted">{{ records('scenes').length }}</span></div><button v-for="scene in records('scenes')" :key="scene.id" :class="{ active: selectedScene?.id === scene.id }" @click="selectScene(scene)"><strong>{{ scene.name }}</strong><span>{{ sceneCues(String(scene.id)).length }} cues · {{ scene.base_stage_preset_id ? 'staged' : 'empty stage' }}</span></button><p v-if="records('scenes').length === 0" class="muted">Add a room to define its backdrop, music, cues, and staging.</p></aside><section v-if="selectedScene" class="scene-detail stack"><header class="scene-detail-header"><div><div class="eyebrow">Room sheet</div><input class="scene-title-input" :value="selectedScene.name" :aria-label="'Scene name ' + selectedScene.name" @input="selectedScene.name = inputValue($event); queueWrite('scenes', selectedScene, ['name'])"></div><button class="danger" :disabled="busy" @click="remove('scenes', selectedScene)">Remove room</button></header><div class="scene-field-grid"><label>Primary backdrop<select :value="selectedScene.primary_backdrop_asset_id || ''" aria-label="Primary backdrop" @change="selectedScene.primary_backdrop_asset_id = selectValue($event) || null; queueWrite('scenes', selectedScene, ['primary_backdrop_asset_id']); loadAssetUrl(String(selectedScene.primary_backdrop_asset_id || ''))"><option value="">No primary backdrop</option><option v-for="asset in readyImages" :key="asset.id" :value="asset.id">{{ title(asset) }}</option></select></label><label>Default music<select :value="selectedScene.default_music_cue_id || ''" aria-label="Default music" @change="selectedScene.default_music_cue_id = selectValue($event) || null; queueWrite('scenes', selectedScene, ['default_music_cue_id'])"><option value="">No default music</option><option v-for="cue in records('audio_cues').filter((cue) => cue.kind === 'music')" :key="cue.id" :value="cue.id">{{ title(cue) }}</option></select></label><label>Base stage<select :value="selectedScene.base_stage_preset_id || ''" aria-label="Base stage preset" @change="selectedScene.base_stage_preset_id = selectValue($event) || null; stagePresetId = String(selectedScene.base_stage_preset_id || ''); queueWrite('scenes', selectedScene, ['base_stage_preset_id'])"><option value="">Empty stage</option><option v-for="preset in records('stage_presets')" :key="preset.id" :value="preset.id">{{ preset.name }}</option></select></label><label>Transition<select :value="selectedScene.transition" aria-label="Transition" @change="selectedScene.transition = selectValue($event); queueWrite('scenes', selectedScene, ['transition'])"><option value="cut">Cut</option><option value="fade_black">Fade through black</option><option value="cross_dissolve">Cross dissolve</option></select></label><label>Duration<input :value="selectedScene.transition_duration_ms" type="number" min="0" max="30000" step="100" aria-label="Transition duration in milliseconds" @change="selectedScene.transition_duration_ms = Number(inputValue($event)); queueWrite('scenes', selectedScene, ['transition_duration_ms'])"></label></div><div class="scene-action-row"><button class="secondary" @click="openSceneModal('character')">Add character</button><button class="secondary" @click="openSceneModal('stage-entry')">Place existing character</button><button class="secondary" @click="openSceneModal('backdrop')">Add backdrop</button><button class="secondary" @click="openSceneModal('stage')">Create stage</button></div><section class="scene-preview-grid"><div class="scene-backdrop-preview" :style="assetUrls[String(selectedScene.primary_backdrop_asset_id)] ? { backgroundImage: 'url(' + assetUrls[String(selectedScene.primary_backdrop_asset_id)] + ')' } : {}"><span v-if="!selectedScene.primary_backdrop_asset_id" class="muted">No backdrop selected</span></div><div class="composer-panel"><div class="row"><h3>Stage</h3><button class="secondary" @click="openSceneModal('stage-entry')">Add performer</button></div><div class="stage-composer" aria-label="Stage composer canvas"><button v-for="entry in stageEntries" :key="entry.id" class="stage-token" :style="{ left: (Number(entry.position_x) * 100) + '%', top: (Number(entry.position_y) * 100) + '%', transform: 'translate(-50%, -50%) scale(' + Number(entry.scale) + ')' }" @pointerdown="beginDrag('stage-preset-entries', entry, $event)">{{ records('npcs').find((npc) => npc.id === entry.npc_id)?.name || 'NPC' }}</button><p v-if="stagePresetId && stageEntries.length === 0" class="muted">This preset has no performers yet.</p><p v-if="!stagePresetId" class="muted">Choose a base stage for this room.</p></div></div></section><section class="cue-shelf stack"><div class="row"><div><h3>Room cues</h3><p class="muted">Audio and video tied to this room stay available from its sheet.</p></div><div class="row"><select v-model="sceneAttachCueIds[String(selectedScene.id)]" :aria-label="'Attach existing cue to ' + selectedScene.name"><option value="">Attach existing cue</option><option v-for="cue in attachableCues(String(selectedScene.id))" :key="cue.id" :value="cue.id">{{ title(cue) }} · {{ cueType(cue) }} · {{ cueScope(cue) }}</option></select><button class="secondary" :disabled="busy || !sceneAttachCueIds[String(selectedScene.id)]" @click="attachCueToScene(String(selectedScene.id))">Attach</button></div></div><div class="scene-cue-list"><article v-for="cue in sceneCues(String(selectedScene.id))" :key="cue.id" class="asset"><div><strong>{{ cue.name }}</strong><div class="muted">{{ cueType(cue) }}</div></div><div class="row"><button class="secondary" :disabled="busy" @click="makeCueGlobal(cue)">Make global</button><button class="danger" :disabled="busy" @click="remove(cueResource(cue), cue)">Remove</button></div></article><p v-if="sceneCues(String(selectedScene.id)).length === 0" class="muted">No cues are bundled with this room yet.</p></div><div class="cue-create-grid"><form class="stack compact" @submit.prevent="createSceneAudio(String(selectedScene.id))"><h4>New sound</h4><input v-model="sceneCueForm(String(selectedScene.id)).name" maxlength="120" :aria-label="'New sound for ' + selectedScene.name" placeholder="Sound or music name"><select v-model="sceneCueForm(String(selectedScene.id)).assetId" :aria-label="'Audio file for ' + selectedScene.name"><option value="">Choose audio</option><option v-for="asset in readyAudio" :key="asset.id" :value="asset.id">{{ title(asset) }}</option></select><select v-model="sceneCueForm(String(selectedScene.id)).kind" :aria-label="'Sound type for ' + selectedScene.name"><option value="music">Music</option><option value="sfx">Sound effect</option></select><button :disabled="busy || !sceneCueForm(String(selectedScene.id)).name.trim() || !sceneCueForm(String(selectedScene.id)).assetId">Create sound</button></form><form class="stack compact" @submit.prevent="createSceneVideo(String(selectedScene.id))"><h4>New video</h4><input v-model="sceneCueForm(String(selectedScene.id)).videoName" maxlength="120" :aria-label="'New video for ' + selectedScene.name" placeholder="Video cue name"><select v-model="sceneCueForm(String(selectedScene.id)).videoAssetId" :aria-label="'Video file for ' + selectedScene.name"><option value="">Choose video</option><option v-for="asset in readyVideos" :key="asset.id" :value="asset.id">{{ title(asset) }}</option></select><button :disabled="busy || !sceneCueForm(String(selectedScene.id)).videoName.trim() || !sceneCueForm(String(selectedScene.id)).videoAssetId">Create video</button></form></div></section><section class="cue-shelf stack"><div class="row"><h3>Alternate backdrops</h3><button class="secondary" @click="openSceneModal('backdrop')">Add alternate</button></div><article v-for="backdrop in sceneBackdrops(String(selectedScene.id))" :key="backdrop.id" class="asset"><strong>{{ backdrop.name }}</strong><span class="muted">{{ title(assets.find((asset) => asset.id === backdrop.asset_id) || backdrop) }}</span></article><p v-if="sceneBackdrops(String(selectedScene.id)).length === 0" class="muted">No alternate backdrops yet.</p></section></section></div></section>
            ...
            <div v-if="sceneModal && selectedScene" class="modal-backdrop" role="presentation" @click.self="closeSceneModal"><section class="modal-panel stack" role="dialog" aria-modal="true"><header class="row"><div><div class="eyebrow">{{ selectedScene.name }}</div><h2 v-if="sceneModal === 'character'">Add character</h2><h2 v-if="sceneModal === 'stage-entry'">Place character</h2><h2 v-if="sceneModal === 'backdrop'">Add backdrop</h2><h2 v-if="sceneModal === 'stage'">Create room stage</h2></div><button class="secondary" @click="closeSceneModal">Close</button></header><form v-if="sceneModal === 'character'" class="stack" @submit.prevent="submitSceneCharacter"><input v-model="sceneCharacterForm.name" maxlength="120" aria-label="Character name" placeholder="Character name"><select v-model="sceneCharacterForm.assetId" aria-label="Character image"><option value="">Choose character image</option><option v-for="asset in readyImages" :key="asset.id" :value="asset.id">{{ title(asset) }}</option></select><input v-model="sceneCharacterForm.pronouns" maxlength="120" aria-label="Pronouns" placeholder="Pronouns"><textarea v-model="sceneCharacterForm.description" maxlength="500" aria-label="Public description" placeholder="Public description"></textarea><div class="segmented"><label><input v-model="sceneCharacterForm.facing" type="radio" value="left"> Facing left</label><label><input v-model="sceneCharacterForm.facing" type="radio" value="right"> Facing right</label></div><label class="check-row"><input v-model="sceneCharacterForm.placeOnStage" type="checkbox"> Place on this room stage</label><button :disabled="busy || !sceneCharacterForm.name.trim() || !sceneCharacterForm.assetId">{{ busy ? 'Adding...' : 'Add character' }}</button></form><form v-if="sceneModal === 'stage-entry'" class="stack" @submit.prevent="submitSceneStageEntry"><select v-model="sceneStageEntryForm.npcId" aria-label="Character"><option value="">Choose character</option><option v-for="npc in records('npcs')" :key="npc.id" :value="npc.id">{{ npc.name }}</option></select><select v-model="sceneStageEntryForm.npcStateId" aria-label="Character appearance"><option value="">Normal appearance</option><option v-for="state in npcStates(sceneStageEntryForm.npcId)" :key="state.id" :value="state.id">{{ state.name }}</option></select><div class="scene-field-grid"><label>X position<input v-model.number="sceneStageEntryForm.positionX" type="number" min="0" max="1" step=".01"></label><label>Y position<input v-model.number="sceneStageEntryForm.positionY" type="number" min="0" max="1" step=".01"></label><label>Scale<input v-model.number="sceneStageEntryForm.scale" type="number" min=".1" max="5" step=".1"></label></div><div class="segmented"><label><input v-model="sceneStageEntryForm.facing" type="radio" value="left"> Facing left</label><label><input v-model="sceneStageEntryForm.facing" type="radio" value="right"> Facing right</label></div><button :disabled="busy || !sceneStageEntryForm.npcId">{{ busy ? 'Placing...' : 'Place character' }}</button></form><form v-if="sceneModal === 'backdrop'" class="stack" @submit.prevent="submitSceneBackdrop"><input v-model="sceneBackdropForm.name" maxlength="120" aria-label="Backdrop name" placeholder="Backdrop name"><select v-model="sceneBackdropForm.assetId" aria-label="Backdrop image"><option value="">Choose image</option><option v-for="asset in readyImages" :key="asset.id" :value="asset.id">{{ title(asset) }}</option></select><button :disabled="busy || !sceneBackdropForm.name.trim() || !sceneBackdropForm.assetId">{{ busy ? 'Adding...' : 'Add backdrop' }}</button></form><form v-if="sceneModal === 'stage'" class="stack" @submit.prevent="submitSceneStage"><input v-model="sceneStageForm.name" maxlength="120" aria-label="Stage name" placeholder="Stage name"><label>Tween duration<input v-model.number="sceneStageForm.tweenDurationMs" type="number" min="0" max="30000" step="100"></label><select v-model="sceneStageForm.tweenEasing" aria-label="Tween easing"><option value="linear">Linear</option><option value="ease_in">Ease in</option><option value="ease_out">Ease out</option><option value="ease_in_out">Ease in and out</option></select><button :disabled="busy || !sceneStageForm.name.trim()">{{ busy ? 'Creating...' : 'Create stage' }}</button></form></section></div>
        </main>
