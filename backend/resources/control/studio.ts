    ...
    setup() {
        ...
        const previewCampaign = async (): Promise<void> => {
            ...
            try {
                ...
                await router.push({ path: `/campaigns/${campaignId}/sessions`, query: { session: session.data.id } });
            } catch (reason) {
    ...
    template: `
        <main v-if="studio" class="studio-shell">
            ...
            <section class="studio-main"><header class="studio-header"><div><div class="eyebrow">Campaign studio</div><h1>{{ studio.campaign.name }}</h1></div><div class="row"><button class="secondary" :disabled="history.length === 0 || saving === 'saving'" @click="undo">Undo</button><button class="secondary" :disabled="redoHistory.length === 0 || saving === 'saving'" @click="redo">Redo</button><button @click="active = 'publish'">Review & publish</button></div></header>
                ...
                <section v-if="active === 'play'" class="studio-content stack"><header class="section-heading"><div><div class="eyebrow">Take it live</div><h2>Session launcher</h2></div></header><article class="review-card"><h3>Preview this campaign</h3><p class="muted">Open a private fresh session from the current draft. A snapshot is created automatically so you can test scenes, music, maps, fog, and presentation controls before inviting players.</p><div class="row"><button :disabled="busy || saving === 'saving'" @click="previewCampaign">{{ busy ? 'Preparing preview…' : 'Preview campaign' }}</button><button class="secondary" @click="openSessions">Open session launcher</button></div></article></section>
            </section>
