...
export const CampaignStudioView = defineComponent({
    setup() {
        ...
        const libraryFile = ref<File | null>(null);
        ...
        const chooseLibraryFile = (event: Event): void => { libraryFile.value = event.target instanceof HTMLInputElement ? event.target.files?.[0] ?? null : null; };
        ...
        const uploadLibraryFile = async (): Promise<void> => {
            if (!libraryFile.value) return;
            const file = libraryFile.value;
            const kind = file.type.startsWith('image/') ? 'image' : file.type.startsWith('audio/') ? 'audio' : file.type.startsWith('video/') ? 'video' : null;
            if (!kind) { error.value = 'Choose an image, audio, or video file.'; return; }
            busy.value = true; error.value = '';
            try { await uploadAsset(file, kind); libraryFile.value = null; }
        ...
        onMounted(load);
        return { sections: studioSections, active, assets, activeAssets, archivedAssets, readyImages, readyAudio, readyVideos, stageEntries, selectedSceneId, selectedScene, mapTokens, selectedMap, selectedFog, filteredCueLibrary, studio, saving, error, busy, history, redoHistory, stagePresetId, mapId, fogAssetId, sceneModal, cueModalOpen, cueEditor, cueEditorAssets, cueFile, backdropFile, backdropUploadModalOpen, sceneForm, sceneCharacterForm, sceneBackdropForm, sceneStageEntryForm, cueSearch, cueScopeFilter, cueTypeFilter, cueSceneFilter, assetUrls, collectionName, libraryFile, mapFile, mapDraft, mapTokenDraft, playerCharacterDraft, npcDraft, castCreationKind, characterArtUploadModalOpen, characterArtUploadFile, characterArtUploadTarget, characterArtUploadNpcId, characterArtUploadEmotion, cueDraft, cueDraftAssets, summary, records, title, inputValue, inputChecked, selectValue, textareaValue, cueType, cueResource, cueScope, sceneCues, sceneBackdrops, npcStates, isNpc, characterArtId, characterInitial, characterKind, emotionNames, emotionAssetId, setEmotionAsset, emotionKitComplete, saveEmotionKit, npcStateDraft, addNpcState, openCharacterArtUpload, closeCharacterArtUpload, chooseCharacterArtUpload, uploadCharacterArt, openSceneModal, closeSceneModal, openBackdropUploadModal, closeBackdropUploadModal, openCueModal, closeCueModal, selectScene, loadAssetUrl, queueWrite, undo, redo, addCollection, updateCollectionMembership, beginDrag, setFog, chooseLibraryFile, chooseMapFile, chooseCueFile, chooseBackdropFile, uploadLibraryFile, createMap, uploadMapFile, createMapToken, createCharacter, createCue, uploadBackdropFile, uploadCueFile, saveCue, makeCueGlobal, submitScene, submitSceneCharacter, submitSceneBackdrop, submitSceneStageEntry, remove, archiveAsset, deleteAsset, publish, back: () => router.push('/'), openLegacy: (section: string) => router.push(`/campaigns/${campaignId}/${section}`), openSessions: () => router.push(`/campaigns/${campaignId}/sessions`) };
    },
    template: `
                ...
                <section v-if="active === 'maps'" class="studio-content stack">
                    ...
                    <div class="studio-split"><aside class="studio-list"><button v-for="map in records('maps')" :key="map.id" :class="{ active: mapId === map.id }" @click="mapId = map.id; loadAssetUrl(String(map.image_asset_id))">{{ map.name }}</button><p v-if="records('maps').length === 0" class="muted">Your map library is empty.</p></aside>
                        <section v-if="selectedMap" class="composer-panel"><div class="map-composer" :style="assetUrls[String(selectedMap.image_asset_id)] ? { backgroundImage: 'url(' + assetUrls[String(selectedMap.image_asset_id)] + ')' } : {}"><button v-for="token in mapTokens" :key="token.id" class="map-editor-token" :style="{ left: (Number(token.position_x) * 100) + '%', top: (Number(token.position_y) * 100) + '%', transform: 'translate(-50%, -50%) scale(' + Number(token.scale) + ')' }" @pointerdown="beginDrag('map-tokens', token, $event)">{{ token.label || records('player_characters').find((pc) => pc.id === token.player_character_id)?.name || records('npcs').find((npc) => npc.id === token.npc_id)?.name || 'Token' }}</button></div>
                            ...
                            <section class="map-fog-panel"><header><div class="eyebrow">Fog of war</div><h3>Reveal fog while you play</h3><p class="muted">Fog is painted during an active session so every reveal can respond to the table. Publish this map, start a session, then use the live map editor to reveal or hide areas.</p></header><button class="secondary" @click="openSessions">Open live map editor</button><p v-if="selectedFog" class="muted">This map has a legacy initial-fog asset configured. It starts hidden; use the live editor to reveal areas.</p></section></section></section>
