...
class ControlSceneController extends Controller
{
    ...
    {
        try {
            [$response, $replayed] = $this->scenes->create($campaign, $request->string('command_id')->toString(), $request->integer('expected_revision'), $request->string('name')->toString(), $request->input('control_notes'), $request->input('primary_backdrop_asset_id'), $request->input('default_music_cue_id'), $request->input('default_video_cue_id'), $request->input('base_stage_preset_id'), $request->string('transition')->toString(), $request->integer('transition_duration_ms'));
