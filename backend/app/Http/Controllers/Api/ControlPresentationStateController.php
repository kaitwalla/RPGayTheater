...
use App\Http\Requests\SetPresentationJoinQrRequest;
...
class ControlPresentationStateController extends Controller
{
    ...
    public function setJoinQr(SetPresentationJoinQrRequest $request, string $campaign, string $session): JsonResponse
    {
        try {
            [$response, $replayed] = $this->states->setJoinQr(
                $campaign,
                $session,
                $request->string('command_id')->toString(),
                $request->integer('expected_revision'),
                $request->boolean('show_join_qr'),
            );
        } catch (StalePresentationState $exception) {
            return response()->json(['message' => $exception->getMessage(), 'data' => $exception->state->toApi()], 409);
        }
