    ...
    private function store(string $path, array $package, array &$storedKeys): Campaign
    {
        ...
        try {
            ...
            foreach ($this->records($manifest, 'npcs') as $record) {
                $ids['npcs'][$this->string($record, 'id')] = $this->create(NonPlayerCharacter::class, ['campaign_id' => $campaign->id, 'normal_asset_id' => $this->reference($ids['assets'], $record, 'normal_asset_id'), 'name' => $this->string($record, 'name'), 'pronouns' => $record['pronouns'] ?? null, 'public_description' => $record['public_description'] ?? null]);
            }
