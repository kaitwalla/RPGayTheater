...
class PlayerCharacterService
{
    ...
    public function create(string $campaignId, string $commandId, int $expectedRevision, string $name, ?string $pronouns, ?string $description, ?string $controlNotes, ?string $avatarId): array
    {
        return DB::transaction(function () use ($campaignId, $commandId, $expectedRevision, $name, $pronouns, $description, $controlNotes, $avatarId): array {
            ...
            $character = PlayerCharacter::query()->create(['campaign_id' => $campaignId, 'avatar_asset_id' => $avatarId, 'name' => trim($name), 'pronouns' => $pronouns, 'public_description' => $description, 'control_notes' => $controlNotes, 'sort_order' => (int) PlayerCharacter::query()->where('campaign_id', $campaignId)->max('sort_order') + 1]);
